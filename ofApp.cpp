...
void Walker::update() {
	...
	for (int i = 0; i < 3; i++) {
		...
		this->rotate_deg += this->rotate_direction * 3;
...
void ofApp::setup() {
	...
	ofColor color;
	int number_of_actor = 17;
	for (int i = 0; i < number_of_actor; i++) {

		color.setHsb(ofMap(i, 0, number_of_actor, 0, 255), 130, 255);
		this->walker_list.push_back(Walker(glm::vec2(ofRandom(-300, 300), ofRandom(-300, 300)), color));
	}
}
